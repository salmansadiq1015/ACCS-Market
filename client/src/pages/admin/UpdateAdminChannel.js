...
export default function UpdateAdminChannel() {
  const { auth } = useAuth();
  ...
  const getChannel = async () => {
    try {
      ...
      if (data) {
        ...
        setChannelLink(data?.channel?.channelLink);
        setSubject(data?.channel?.subject);
        setCategory(data?.channel?.category);
        setPrice(data?.channel?.price);
        setSubscriber(data?.channel?.subscriber);
        setMonotization(data?.channel?.monotization);

        setAllowComment(data?.channel?.allowComment);
        setDescription(data?.channel?.description);
        setContentType(data?.channel?.contentType);
        setIncome(data?.channel?.income);
        setExpense(data?.channel?.expense);
        setStatus(data?.channel?.status);
  ...
  return (
    <Layout title={"Create-Channel"}>
      <div className="w-full h-[88vh] overflow-y-auto px-3 sm:pt-6 pb-[3rem] md:pb-[1.5rem]  px-1 sm:px-6 flex items-center flex-col gap-2">
        <div className="mt-3 flex flex-col gap-3 w-[98%] sm:max-w-[70%] py-4 px-2 sm:px-4 rounded-lg shadow-md shadow-gray-300 border ">
          ...
          <form onSubmit={handleUpdate} className="mt-4 flex flex-col gap-4">
            ...
            <select
              ...
              className=" w-full h-[2.7rem] rounded-md shadow-md shadow-gray-200 border-2 border-gray-700 outline-none px-3 "
            >
              <option value="None">--Update Verification Status--</option>
